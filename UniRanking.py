...
min_students, max_students = df["No of student"].str.replace(',', '').astype(int).min(), df["No of student"].str.replace(',', '').astype(int).max()
students_range = st.sidebar.slider("Number of Students", min_students, max_students, (min_students, max_students))

# Filter by Overall Score
min_score, max_score = df["OverAll Score"].astype(float).min(), df["OverAll Score"].astype(float).max()
score_range = st.sidebar.slider("Overall Score", min_score, max_score, (min_score, max_score))

# Filter the DataFrame based on user selections
filtered_df = df[(df["Location"] == selected_location) &
                 (df["No of student"].str.replace(',', '').astype(int) >= students_range[0]) &
                 (df["No of student"].str.replace(',', '').astype(int) <= students_range[1]) &
                 (df["OverAll Score"].astype(float) >= score_range[0]) &
                 (df["OverAll Score"].astype(float) <= score_range[1])]
